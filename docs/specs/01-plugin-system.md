# 01 -- Plugin System: Manifest & Plugin Class
...
```tsx
import { useWidgetConfig, useWidgetPlugin, useService, useUI } from '@vibe-ctl/plugin-api';
...
```typescript
declare module '@vibe-ctl/plugin-api' {
...
- **`ctx.ui` is types-only in `@vibe-ctl/plugin-api`; components come
