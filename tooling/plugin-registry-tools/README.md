# vibe-ctl-plugin-registry-tools
...
```bash
pnpm add -g vibe-ctl-plugin-registry-tools
# or run ad-hoc:
npx vibe-ctl-plugin-registry-tools <command>
```
...
| `validate <plugin.json>` | Validate a manifest against the Zod schema exported by `@vibe-ctl/extension-api`. |
